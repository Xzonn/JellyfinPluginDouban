    ...
    public async Task<IEnumerable<RemoteImageInfo>> GetImages(BaseItem item, CancellationToken token)
    {
        ...
        if (!string.IsNullOrEmpty(subject.PosterId))
        {
            var image = new RemoteImageInfo()
            {
                ...
                ThumbnailUrl = $"{Configuration.CdnServer}/view/photo/s_ratio_poster/public/{subject.PosterId}.webp",
                Type = ImageType.Primary,
                Url = $"{Configuration.CdnServer}/view/photo/l/public/{subject.PosterId}.webp",
            };
            images.Add(image);
        }
